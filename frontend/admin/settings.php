...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_org_key') {
    $newKey = trim($_POST['org_key'] ?? '');
    if ($newKey !== '') {
        $db->prepare("UPDATE tf_organizations SET org_key = ? WHERE id = ?")->execute([$newKey, currentUser()['org_id']]);
        $msg = "Organization Key completely updated.";
    }
}

$orgKeyQuery = $db->prepare("SELECT org_key FROM tf_organizations WHERE id = ?");
$orgKeyQuery->execute([currentUser()['org_id']]);
$currentOrgKey = $orgKeyQuery->fetchColumn();
?>
</head>
                ...
                <div class="set-wrap a2">
                    ...
                    <div class="set-main">
                        ...
                        <div id="panel-security" class="set-panel <?= $activeTab === 'security' ? 'active' : '' ?>">
                            <div class="set-title">Security & Organization Key</div>
                            <p style="color:var(--text3); font-size:14px; margin-bottom:20px;">Protect your company namespace by configuring an Organization Passkey. Managers and Admins will be required to enter this key when adding new users.</p>
                            
                            <form method="POST" action="?tab=security" style="background:rgba(255,255,255,0.02); padding:24px; border-radius:12px; border:1px solid rgba(255,255,255,0.05); margin-bottom:30px;">
