...
<head>
    ...
    // For managers, show activity related to their projects and team members
    ...
    $sql = "SELECT a.*, u.name uname FROM tf_activity a LEFT JOIN tf_users u ON a.user_id=u.id LEFT JOIN tf_projects p ON a.project_id=p.id WHERE (p.manager_id=? OR u.org_id=? OR p.org_id=?)";
    $params = [$uid, $orgId, $orgId];
    if ($pid > 0) {
        ...
        <div class="tf-main">
            <div class="tf-topbar">
                <div style="display:flex; align-items:center; gap:20px;">
                    <div class="tf-topbar-title">Project Activity Log</div>
                    <form method="GET" style="margin:0;">
                        <select name="project" onchange="this.form.submit()" class="tf-inp" style="padding:6px 12px; border-radius:8px; border:1px solid var(--border); background:var(--surface2);">
                            <option value="0">All Managed Projects Comb.</option>
            ...
            <div class="tf-page">
                <div class="tf-page-hd a1">
                    <div>
                        <div class="tf-title">Team Activity</div>
                        <div class="tf-subtitle">Recent actions across your managed projects</div>
                    </div>
                </div>
